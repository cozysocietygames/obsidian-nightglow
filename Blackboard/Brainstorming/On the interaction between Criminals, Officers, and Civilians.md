...
> [!example] Suggestion: Lookouts (Core Mechanic) (Model 2)
...
>A player may hire an Officer to guard them preemptively. The Officer dispatches a number of Pokemon to each serve as Lookouts for the player. Each Lookout stops the first Stakeout Timer that some Criminal attempts to start.
>
>#### Guarding
>If a Stakeout timer is started while a Lookout is *Guarding*, the player is notified right away that a Criminal attempted to mark them as a target, and the Criminal may not repeat the attempt for some amount of time.
>
