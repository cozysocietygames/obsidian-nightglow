...
Come to think of it, why don't we do the same thing for `BodyPlan` and `Integument`, make them components of a `SpeciesBodyEntity`  instead? Then the `SpeciesEntity` could just have a property `body` that takes an id of a `SpeciesBodyEntity` as its value? Please let me know if this thinking is faulty.  
